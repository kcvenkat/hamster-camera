...
for name, img in hamster_images.items():
    if img is None:
...
while True:
    ...
    if not hand_results.hand_landmarks:
        ...
        for i, hand in enumerate(hand_results.hand_landmarks):
            result = fingers_together(hand, w, h)
            print(f"hand {i}: fingers_together = {result}")

        purse_hand_ok = any(
            fingers_together(hand, w, h) for hand in hand_results.hand_landmarks
        )

    print("purse_hand_ok:", purse_hand_ok)
